// 기말 보고서 docx 생성 (report-template.docx 15섹션 + 그림별 캡처 가이드)
// 실행: NODE_PATH=$(npm root -g) node scripts/build_report.js → docs/final-report.docx
const fs = require("fs");
...
const {
  ...
  ShadingType, PageNumber, Footer, PageBreak, HeightRule,
} = require(M);
...
C(...fig({
  caption: "혼동행렬(Confusion Matrix) — 카테고리별 분류 결과",
...
Packer.toBuffer(doc).then((buf) => { fs.writeFileSync(path.join(ROOT, "docs", "final-report.docx"), buf); console.log("OK → docs/final-report.docx (" + buf.length + " bytes), 그림 수=" + FIG); });
